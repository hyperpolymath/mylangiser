...
Mylangiser is a progressive-disclosure interface generator. It analyses
complex API surfaces, assigns cognitive-load complexity scores, and
generates layered wrappers at three disclosure levels (@beginner,
@intermediate, @expert).
...
Thresholds (configurable in mylangiser.toml): - 0-33: @beginner - 34-66:
@intermediate - 67-100: @expert
...
[arabic]
. *Layer ordering*: Beginner < Intermediate < Expert (never reversed)
. *Parameter monotonicity*: beginner_params <= intermediate_params <=
expert_params
. *Score bounds*: 0 <= complexity_score <= 100
. *Default safety*: smart defaults satisfy parameter type constraints
