...
mylangiser generates progressive-disclosure interfaces for complex APIs
via My-Lang. It analyses API complexity, scores each endpoint, and
assigns disclosure levels (beginner / intermediate / expert), then
generates layered wrapper code with smart defaults for omitted
parameters. The goal is to make any API learnable by newcomers while
remaining fully expressive for experts. mylangiser targets library and
API authors who want to reduce the cognitive load on users without
sacrificing power.
